...
//	makeRectangle(379, 80, 68, 68);
	...
	startW = round(destW * ratio);

	startH = round(destH * ratio);

	startX = round(incenterX - startW/2);

	startY = round(incenterY - startH/2);
	...
	if (oriOut == true) {
		...
		oriW = round(inW * scalefactor);

		oriH = round(inH * scalefactor);
	...
	for (p = 0; p < step ; p++) {
		...
		if (oriOut == true) {
			...
			oX = round(currX * scalefactor);

			oY = round(currY * scalefactor);

			oW = round(currW * scalefactor);

			oH = round(currH * scalefactor);
